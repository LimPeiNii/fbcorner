    ...
    if (isset($_POST['submit_check_staff'])){
        ...
        elseif (strlen($email) > 96 || !filter_var($email, FILTER_VALIDATE_EMAIL))
            ...
            if (isset($_POST['staff_id']))
                $sql       = "SELECT email FROM staff WHERE email = '" . $email . "' AND staff_id != " . $_POST['staff_id'];
            else
        ...
        elseif (!preg_match($tel_pattern, $contact_num) && !preg_match($tel_pattern_2, $contact_num))
            ...
            if (isset($_POST['staff_id']))
                $sql       = "SELECT contact_num FROM staff WHERE contact_num = '" . $contact_num . "' AND staff_id != " . $_POST['staff_id'];
            else
                $sql       = "SELECT contact_num FROM staff WHERE contact_num = '" . $contact_num . "'";

            $statement = $connection->query($sql);

            if ($statement->num_rows > 0)
                $json['error']['contact-num'] = "The contact number ($email) has been used by another staff!";
        }
