<?php 
    ...
    if (isset($_SESSION['login_rest_id']) && isset($_SESSION['login_user_id'])) {
        ...
        $has_permission = checkUserPermission($connection, $_SESSION['login_user_id'], 'reservation/table', 'access_permission');
        $has_modify_permission = checkUserPermission($connection, $_SESSION['login_user_id'], 'reservation/table', 'modify_permission');
    ...
    <div class="side-bar-body">
        ...
        <div class="content p-4">
            <?php if ((int)$has_permission['has_permission']){ ?>
                ...
                <a href="table_form.php"><button type="button" class="btn btn-secondary float-end" data-bs-toggle="tooltip" data-bs-placement="top" title="Add"><i class="fas fa-plus font-size-20 text-white m-0 p-0" style="line-height: 30px;"></i></button></a>
                ...
                <div class="table-responsive table-list">
                    <?php if (count($tables) == 0 || count($tables) == 1) { ?>
                    ...
                    <?php } ?>
                        <thead>
                            <tr>
                                ...
                                <th width="25%" class="px-3">Table No. (Name)</td>
                                <th width="20%" class="px-3">Capacity</td>
                                <th width="20%" class="px-3">Status</td>
                                <th width="20%" class="px-3">Sort Order</td>
                        ...
                        <tbody>
                            <?php foreach ($tables as $table) { ?>
                                <tr>
                                    <td style="width: 20px;" class="px-4 align-middle"><input type="checkbox" class="form-check-input checkbox-default data-checkbox" value="<?=$table['table_id']?>"></td>
                                    <td width="25%" class="align-middle px-3"><?=$table['table_num_name']?></td>
                                    <td width="20%" class="align-middle px-3"><?=$table['capacity']?></td>
                                    <td width="20%" class="align-middle px-3"><?=$table['status'] ? 'Available' : 'Unavailable'?></td>
                                    <td width="20%" class="align-middle px-3"><?=$table['sort_order']?></td>
                                    <td width="10%" class="text-center">
                                        <a href="table_form.php?table_id=<?=$table['table_id']?>"><button type="button" class="btn bg-primary bg-opacity-75" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><i class="fas fa-pen font-size-20 text-white m-0 p-0" style="line-height: 25px;"></i></button></a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
            ...
            function delete_data(){
                ...
                if (selected_data.length > 0){
                    <?php if ((int)$has_modify_permission['has_permission']){ ?>
                        if (confirm("Are you sure you want to delete the table(s)?\n* Relevant reservations and pre-orders will be disabled")){
                            $.ajax({ 
                                url: '../../helpers/table.php',
                                data: {selected_group_table: selected_data},
                                type: 'post',
                                success: function(){
                    ...
                    <?php } else { ?>
                        close_message();
                        $('<div class="alert form-message mt-4 mb-0 alert-danger" role="alert"><button type="button" onclick="close_message();" class="btn-close btn float-end" aria-label="Close"></button><i class="fas fa-exclamation-circle"></i>&nbsp;&nbsp;Sorry, you do not have the permission to delete the table(s).</div>').insertBefore('.table-list');
                    <?php } ?>
                }
            }
