<?php
    ...
    $data = $_SESSION['send_email']['order_data'];
    ...
    if ($_SESSION['send_email']['type'] == 'add' && $_SESSION['send_email']['to'] == 'customer'){
        $message = "<p>Dear " . trim($order_data['Name']) . ",</p>
        <p>Thank you for your interest in " . $restaurant['rest_name'] . " foods. Your order has been received and will be processed soon.</p>";
    } elseif ($_SESSION['send_email']['type'] == 'add' && $_SESSION['send_email']['to'] == 'restaurant'){
        $message = "<p>You have received an order.</p>";
    } elseif ($_SESSION['send_email']['type'] == 'edit' && $_SESSION['send_email']['to'] == 'customer'){
        $message = "<p>Dear " . trim($order_data['Name']) . ",</p>
        <p>Thank you for your interest in " . $restaurant['rest_name'] . " foods. Your order has an update, as shown below.</p>";
    } elseif ($_SESSION['send_email']['type'] == 'edit' && $_SESSION['send_email']['to'] == 'restaurant'){
        $message = "<p>An order has been updated, as shown below.</p>";
    }
...
<html>
<head>
  <title><?=$restaurant['rest_name']?> &ndash; Order <?=$_SESSION['send_email']['order_id']?><?=$_SESSION['send_email']['type'] == 'edit' ? ' Update' : '' ?></title>
  <style>
        td{ 
...
<body>
    <div style="margin: 1rem;">
        ...
        <table style="margin-left: auto; margin-right: auto; width: 100%; border: 2px solid #dee2e6; border-collapse: collapse;">
            <thead>
                <tr style="background-color: #e2e2e2;"><th colspan="2" style="padding: 0.75rem;" class="data-title"> Order Details</th></tr>
            </thead>
            <tbody class="order-details">
                <tr>
                    <td><span class="data-title">Order ID:</span> <?=$_SESSION['send_email']['order_id']?></td>
                    <td><span class="data-title">Name:</span> <?=trim($order_data['Name'])?></td>
                </tr>
                <tr>
                    <td><span class="data-title">Table Number (Name):</span> <?=$order_data['Table Number (Name)']?></td>
                    <td><span class="data-title">E-mail:</span> <a href="mailto:<?=$customer['email']?>"><?=$customer['email']?></a></td>
                </tr>
                <tr>
                    <td><span class="data-title">Created Date & Time:</span> <?=$order_data['Created Date']?></td>
                    <td><span class="data-title">Contact Number:</span> <?=$customer['contact_num']?></td>
                </tr>
                <?php if ($order_data['Self Pickup Time'] != '-') { ?>
                <tr>
                    <td><span class="data-title">Pickup Date & Time:</span> <?=$order_data['Self Pickup Time']?></td>
                    <td></td>
                </tr>
                <?php } ?>
                <tr>
                    <?php $order_status = substr($order_data['Status'], (strpos($order_data['Status'], '>') + 1));?>
                    <?php $order_status = substr($order_status, 0, (strpos($order_status, '</')));?>
                    <td><span class="data-title">Status:</span> <?=$order_status?></td>
                    <td></td>
                </tr>
                <tr>
