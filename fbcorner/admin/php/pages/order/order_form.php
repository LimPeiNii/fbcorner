<?php 
    ...
    if (isset($_SESSION['login_rest_id']) && isset($_SESSION['login_user_id'])) {
        ...
        $has_permission = checkUserPermission($connection, $_SESSION['login_user_id'], 'order/order', 'modify_permission');
        ...
        if ((int)$has_permission['has_permission']){
            ...
            if (isset($order) && $order['voucher_id'] != '0'){
                ...
                $vouchers = getVouchers($connection, $order['cus_id'], ['valid' => true, 'applied_order_id' => '0', 'not_in_use' => true]);
            } else {
                $vouchers = [];
            }
        }
        ...
        <div class="content p-4">
            <?php if ((int)$has_permission['has_permission']){ ?>
                ...
                <div>
                    ...
                    <?php } else { ?>    
                        <form action="../../helpers/order.php" class="m-5" id="order-form" method="POST">
                    <?php } ?>
                        <div class="mb-4">
                            <input type="text" style="display: none;" name="submit_check_order">
                            <?php if (isset($order)) { ?>
                            <input type="text" style="display: none;" name="order_id" value="<?=$order['order_id']?>">
                            <?php } ?>
                            <?php if (isset($order)) { ?>
                                <input type="text" style="display: none;" name="cus-id" id="cus_id" value="<?=$order['cus_id']?>">
                            <?php } else { ?>
                            ...
                            <?php if (isset($order)) { ?>
                                <input class="form-control" autocomplete="off" list="datalistOptions" name="cus_name" id="cus-name" onchange="addCusID(value);" value="<?=$the_customer['firstname'] . ' ' . $the_customer['lastname']?>" disabled>
                            <?php } else { ?>
                                <input class="form-control" autocomplete="off" list="datalistOptions" name="cus_name" id="cus-name" onchange="addCusID(value);">
                            <?php } ?>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="form-check form-check-inline">
                                <?php if (isset($order)) { ?>
                                    <input class="form-check-input" type="radio" name="order_type" id="dine-in" value="dine_in" <?=($order['table_id'] ? 'checked="checked"' : '')?>>
                                <?php } else { ?>
                                    <input class="form-check-input" type="radio" name="order_type" id="dine-in" value="dine_in" checked="checked">
                                <?php } ?>
                                <label class="form-check-label" for="dine-in">Dine-In</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <?php if (isset($order)) { ?>
                                    <input class="form-check-input" type="radio" name="order_type" id="self-pickup" value="self_pickup" <?=($order['table_id'] ? '' : 'checked="checked"')?>>
                                <?php } else { ?>
                        ...
                        <div class="mb-4 d-none" id="table-container">
                            ...
                            <select class="form-select" id="table-num-name" name="table-num-name">
                                ...
                                <?php foreach ($tables as $the_table) { ?>
                                <?php if (isset($order) && $the_table['status'] == '1' && $order['table_id'] == $the_table['table_id']) { ?>
                                    <option value="<?=$the_table['table_id']?>" selected="selected"><?=$the_table['table_num_name']?></option>
                                <?php } elseif ($the_table['status'] == '1') { ?>
                                    <option value="<?=$the_table['table_id']?>"><?=$the_table['table_num_name']?></option>
                                <?php } ?>
                                <?php } ?>
                            </select>
                            <p class="d-block text-danger"></p>
                        </div>
                        <div class="mb-4 d-none" id="pickup-time-container">
                            <label for="pickup_time" class="form-label fw-bold">Pickup Time <span class="required-star">*</span></label>
                            <input type="time" class="form-control" name="pickup_time" id="pickup_time"<?=(isset($order) && !empty($order['pickup_time']) ? ' value="' . date("H:i", strtotime($order['pickup_time'])) . '"' : '')?>>
                            <p class="d-block text-danger"></p>
                        </div>
                        <div class="mb-4">
                                ...
                                <div class="tab-pane fade show active p-3" id="nav-offer" role="tabpanel" aria-labelledby="nav-offer-tab">
                                    <nav class="navbar navbar-light">
                                        <div class="container-fluid">
                                            ...
                                            <input type="search" name="" class="form-control" id="search-offers" placeholder="Search">
                                            </div>
                                        </div>
                                    </nav>
                                    <?php if ($promo_codes) { ?>
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select" id="status" name="status">
                                <?php $statuses = ['Pending', 'Processing', 'Served', 'Completed', 'Cancelled']; ?>
        ...
        <script>   
            ...
            function applyOffer(promo_id){
                if ($('#cus-name').val() == ''){
                    ...
                    $('<div class="alert form-message mt-4 mb-0 alert-danger" role="alert"><i class="fas fa-exclamation-circle"></i>&nbsp;&nbsp;Please enter a customer name before applying an offer!<button type="button" onclick="close_message();" class="btn-close btn float-end" aria-label="Close"></button></div>').insertBefore('#order-form');
                    $('#order-form').addClass('mt-4');
                    $('html, body').animate({ scrollTop: 0 }, 0);
                } else if ($('#cus-name').val() != '' && $('#cus_id').val() == ''){
                    $('#offerModal').modal('hide');
                    $('.close-details-modal').trigger('click');
                    $('<div class="alert form-message mt-4 mb-0 alert-danger" role="alert"><i class="fas fa-exclamation-circle"></i>&nbsp;&nbsp;Offers are only limited to registered customer!<button type="button" onclick="close_message();" class="btn-close btn float-end" aria-label="Close"></button></div>').insertBefore('#order-form');
                    $('#order-form').addClass('mt-4');
                    $('html, body').animate({ scrollTop: 0 }, 0);
                } else {
                    $.ajax({
            ...
            function updateVoucherList(cus_id){
                $.ajax({
                    url: '../../helpers/order.php',
                    data: {
                        cus_id: cus_id,
                        update_voucher_list: true
                    },
                    method: 'post',
                    success: function(output){
                        var vouchers = $.parseJSON(output);

                        if (vouchers.length > 0){

                            var html = '';

                            html += '<div class="list-group" style="max-height: 269.4px; min-height: 269.4px; margin: 0.8rem; ' + (vouchers.length <= 3 ? 'overflow:hidden;' : 'overflow: auto;') + ' border: 1px solid black; background-color: white;">';

                        $.each(vouchers, function(voucher_key, voucher) {
                            html +=     '<div style="' + ((voucher_key == vouchers.length-1) && (vouchers.length >= 3) ? 'border-bottom: none; ' : 'border-bottom: 1px solid grey; border-bottom-left-radius: unset; border-bottom-right-radius: unset; ') + 'cursor: pointer; min-height: 89.8px;" class="list-group-item list-group-item-action" onclick="applyRemoveVoucher(' + voucher['equal_price'] + ', ' + voucher['the_voucher_id'] + ', $(this))">';
                            html +=         '<div class="d-flex flex-wrap w-100 p-1">';
                            html +=             '<i class="bi bi-ticket-perforated-fill voucher-icon mx-1 me-4 align-self-center text-success" style="font-size: 30px;"></i>';
                            html +=             '<div class="flex-grow-1 pt-1">';
                            html +=                 '<input type="number" class="d-none" value="' + voucher['the_voucher_id'] + '">';
                            html +=                 '<h4 class="mb-1 voucher-name"><strong>' + voucher['name'] + '</strong></h4>';
                            html +=                 '<small>Valid Till ' + voucher['expired_date'] + '</small>';
                            html +=             '</div>';
                            html +=         '</div>';
                            html +=     '</div>';
                        });
                            ...
                            html += '<div style="margin: 0.8rem; border-radius: 0.25rem; height: 296px;" id="voucher">';
                            html +=   '<table class="m-0 w-100">';
                            html +=       '<tr class="w-100">';
                            html +=           '<td class="text-center align-middle" style="height: 230px;">No available voucher</td>';
                            html +=       '</tr>';
                            html +=   '</table>';
                            html += '</div>';
            ...
            function addCusID(name){
                ...
                if ($('#datalistOptions [value="' + name + '"]').length != 0){
                    ...
                    updateVoucherList(data_array[1]);
                } else {
                    var html = '';
            ...
            $(document).ready(function() {
                ...
                <?php if (isset($order) && $order['table_id'] == '0') { ?>
                    $('#self-pickup').trigger('change');
                <?php } else { ?>
                    $('#dine-in').trigger('change');
                <?php } ?>
                ...
                $(document).on('change', '.qty', function () {
                    ...
                    if (parseFloat(total.text().substring(3)) < parseFloat($('.offer-icon.fa-check-circle').parent().find('input').val())){
                        removeOffer();
                        alert('Offer is disabled as order total is lower than the minimum spend.')
                    } else {
                        resetOfferList();
                ...
                $('#add-food-btn').click(function () {
                    ...
                    if (discount_price){
                        html +=         '<span class="text-danger subtotal_data">RM ' + parseFloat(discount_price).toFixed(2) + '</span>';
                    } else {
                ...
                $("#order-form").submit(function(event){
                    ...
                    $('.form-message').remove();
                    $('input, select').removeClass('is-invalid red-box-shadow');
                    $('.text-danger').not('.total, .subtotal_data').text('');

                    $.ajax({
                        url: '../../helpers/order.php',
                        data: $('#order-form select, #order-form input:not([type=radio]), #order-form textarea, #order-form input[type=radio]:checked'),
                        method: 'post',
                        success: function(output) {
                            var json = $.parseJSON(output);
                            ...
                            if(json['warning']){
                                ...
                                $('<div class="alert form-message mt-4 mb-0 alert-danger" role="alert"><button type="button" onclick="close_message();" class="btn-close btn float-end" aria-label="Close"></button><i class="fas fa-exclamation-circle"></i>&nbsp;&nbsp;There are errors on the form. Please fix them before continuing.</div>').insertBefore('#order-form');
                                $('#order-form').addClass('mt-4');
                                ...
                                $('input[name="submit_check_order"]').attr('name', "submit_form_order");
                                if ($('.grand_total').length != 0){
