<?php 
    ...
    if (isset($_SESSION['login_rest_id']) && isset($_SESSION['login_user_id'])) {
        ...
        $has_permission = checkUserPermission($connection, $_SESSION['login_user_id'], 'order/order', 'access_permission');
        $has_modify_permission = checkUserPermission($connection, $_SESSION['login_user_id'], 'order/order', 'modify_permission');
    ...
    <div class="side-bar-body">
        ...
        <div class="content p-4">
            <?php if ((int)$has_permission['has_permission']){ ?>
                ...
                <a href="order_form.php"><button type="button" class="btn btn-secondary float-end" data-bs-toggle="tooltip" data-bs-placement="top" title="Add"><i class="fas fa-plus font-size-20 text-white m-0 p-0" style="line-height: 30px;"></i></button></a>
                    ...
                    <?php } ?>
                        <thead>
                            <tr class="text-center align-middle">
                                ...
                                <th width="20%" class="px-3">Date & Time</td>
                                <th width="10%" class="px-3">Table No. (Name)</td>
                                <th width="10%" class="px-3">Self Pickup</td>
                        ...
                        <tbody>
                            <?php 
                                ...
                                foreach ($orders as $order) {
                                    if (($order['status'] == 'Cancelled' || $order['status'] == 'Disabled') && $order['reservation_id'] != '0')
                                        continue;
                                    $counter++;
                            ...
                            ?>
                                <tr class="text-center align-middle">
                                    ...
                                    <td width="20%" class="px-3"><?=date('d/m/Y h:i a', strtotime($order['datetime']))?></td>
                                    <td width="10%" class="px-3"><?=$table_num_name?></td>
                                    <?php if ($table_num_name == '-') { ?>
                                    <td width="10%" class="px-3"><i class="fas fa-check"></i></td>
                                    <?php } else { ?>
                                    <td width="10%" class="px-3"></td>
                                    <?php } ?>
                                    <?php if (strlen($order['additional_notes']) > 0) { ?>
                                    <td width="10%" class="px-3"><i class="fas fa-check"></i></td>
                                    <?php } else { ?>
                                    <td width="10%" class="px-3"></td>
                                    <?php } ?>
                                    ...
                                    <?php } else if ($order['status'] == 'Served') { ?>
                                    <td width="10%" class="px-3"><span class="badge bg-success font-size-14">&nbsp;&nbsp;&nbsp;&nbsp;Served&nbsp;&nbsp;&nbsp;&nbsp;</span></td>
                                    <?php } else if ($order['status'] == 'Disabled') { ?>
                                    ...
                                    <td width="10%" class="px-3"><span class="badge bg-warning font-size-14 text-dark bg-opacity-75">&nbsp;&nbsp;&nbsp;Pending&nbsp;&nbsp;&nbsp;</span></td>
                                    <?php } else if ($order['status'] == 'Processing') { ?>
                                    <td width="10%" class="px-3"><span class="badge bg-info font-size-14 bg-opacity-50" style="color: #055160;">Processing</span></td>
                                    <?php } ?>
                                    <td width="13%">
                                        <?php if ($order['status'] == 'Disabled' || $order['status'] == 'Completed' || $order['status'] == 'Cancelled') { ?>
                                        <?php if ($order['reservation_id'] == '0') { ?>
                                            <button type="button" class="btn bg-success bg-opacity-75" onclick="viewDetails(<?=$order['order_id']?>);" data-bs-toggle="tooltip" data-bs-placement="top" title="View"><i class="fas fa-eye font-size-20 text-white m-0 p-0" style="line-height: 25px;"></i></button>
                                        <?php } else { ?>
                                            <button type="button" class="btn bg-success bg-opacity-75" onclick="viewPreOrderDetails(<?=$order['order_id']?>, <?=$order['reservation_id']?>);" data-bs-toggle="tooltip" data-bs-placement="top" title="View"><i class="fas fa-eye font-size-20 text-white m-0 p-0" style="line-height: 25px;"></i></button>
                                        <?php } ?>
                                        ...
                                        <?php if ($order['reservation_id'] == '0') { ?>
                                            <button type="button" class="btn bg-success bg-opacity-75" onclick="viewDetails(<?=$order['order_id']?>);" data-bs-toggle="tooltip" data-bs-placement="top" title="View"><i class="fas fa-eye font-size-20 text-white m-0 p-0" style="line-height: 25px;"></i></button>
                                            <a href="order_form.php?order_id=<?=$order['order_id']?>"><button type="button" class="btn bg-primary bg-opacity-75" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><i class="fas fa-pen font-size-20 text-white m-0 p-0" style="line-height: 25px;"></i></button></a>
                                        <?php } else { ?>
                                            <button type="button" class="btn bg-success bg-opacity-75" onclick="viewPreOrderDetails(<?=$order['order_id']?>, <?=$order['reservation_id']?>);" data-bs-toggle="tooltip" data-bs-placement="top" title="View"><i class="fas fa-eye font-size-20 text-white m-0 p-0" style="line-height: 25px;"></i></button>
        ...
        <script>
            ...
            function delete_data(){
                ...
                if (selected_data.length > 0){
                    ...
                    <?php } else { ?>
                        close_message();
                        $('<div class="alert form-message mt-4 mb-0 alert-danger" role="alert"><button type="button" onclick="close_message();" class="btn-close btn float-end" aria-label="Close"></button><i class="fas fa-exclamation-circle"></i>&nbsp;&nbsp;Sorry, you do not have the permission to delete the order(s).</div>').insertBefore('.order-list');
                    <?php } ?>
                }
            }
            ...
            $(document).ready(function() {
                ...
                var dataTable = $('.order-list table').DataTable();
