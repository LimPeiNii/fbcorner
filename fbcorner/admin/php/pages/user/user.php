<?php 
    ...
    if (isset($_SESSION['login_rest_id']) && isset($_SESSION['login_user_id'])) {
        ...
        $has_permission = checkUserPermission($connection, $_SESSION['login_user_id'], 'user/user', 'access_permission');
        $has_modify_permission = checkUserPermission($connection, $_SESSION['login_user_id'], 'user/user', 'modify_permission');
    
        if ((int)$has_permission['has_permission']){
            $users = getUsers($connection, $_SESSION['login_rest_id']);
        }
...
<body>
    ...
    <div class="side-bar-body">
        ...
        <div class="content p-4">
            <?php if ((int)$has_permission['has_permission']){ ?>
                <button type="button" onclick="delete_data();" class="btn bg-danger bg-opacity-75 float-end ms-2 bs-tooltip" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete"><i class="fas fa-trash-alt font-size-20 text-white m-0 p-0" style="line-height: 30px;"></i></button>
                <a href="user_form.php"><button type="button" class="btn btn-secondary float-end bs-tooltip" data-bs-toggle="tooltip" data-bs-placement="top" title="Add"><i class="fas fa-plus font-size-20 text-white m-0 p-0" style="line-height: 30px;"></i></button></a>
                ...
                <div class="table-responsive user-list">
                    <?php if (count($users) == 0 || count($users) == 1) { ?>
                    ...
                    <?php } ?>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    <td style="width: 20px;" class="px-4 align-middle"><input type="checkbox" class="form-check-input checkbox-default data-checkbox" value="<?=$user['user_id']?>"></td>
                                    <td width="5%" class="text-center align-middle"><?=$counter?></td>
                                    <td class="align-middle px-3"><?=$user['username']?></td>
                                    <td width="10%" class="text-center">
                                        <a href="user_form.php?user_id=<?=$user['user_id']?>"><button type="button" class="btn bg-primary bg-opacity-75" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><i class="fas fa-pen font-size-20 text-white m-0 p-0" style="line-height: 25px;"></i></button></a>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
        ...
        <script>
            ...
            function delete_data(){
                ...
                if (selected_data.length > 0){
                    ...
                    <?php } else { ?>
                        close_message();
                        $('<div class="alert form-message mt-4 mb-0 alert-danger" role="alert"><button type="button" onclick="close_message();" class="btn-close btn float-end" aria-label="Close"></button><i class="fas fa-exclamation-circle"></i>&nbsp;&nbsp;Sorry, you do not have the permission to delete the user(s).</div>').insertBefore('.user-list');
                    <?php } ?>
                }
            }
