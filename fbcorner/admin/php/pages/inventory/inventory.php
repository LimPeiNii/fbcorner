    ...
    <div class="side-bar-body">
        ...
        <div class="content p-4">
            <?php if ((int)$has_permission['has_permission']){ ?>
                ...
                <button type="button" class="btn btn-info float-end" id="reset-all" data-bs-toggle="tooltip" data-bs-placement="top" title="Refresh Stock"><i class="fas fa-sync-alt font-size-20 text-white m-0 p-0" style="line-height: 30px;"></i></button>
                ...
                $('#reset-all').click(function () {
                    ...
                    if (all_item_id.length > 0){
                        <?php if ((int)$has_modify_permission['has_permission']){ ?>
                            if (confirm("Are you sure you want to refresh the stock for the item(s)?")){
                                $.ajax({ 
                        ...
                        <?php } else { ?>
                            close_message();
                            $('<div class="alert form-message mt-4 mb-0 alert-danger" role="alert"><button type="button" onclick="close_message();" class="btn-close btn float-end" aria-label="Close"></button><i class="fas fa-exclamation-circle"></i>&nbsp;&nbsp;Sorry, you do not have the permission to refresh the inventory stock(s).</div>').insertBefore('.inventory-list');
                        <?php } ?>
                    }
                });
